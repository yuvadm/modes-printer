...
logging.basicConfig(filename='modes.log', filemode='a', level=logging.INFO, format='%(asctime)-15s %(levelname)s %(message)s')
...
def process_jobs(jobs):
    for job in jobs:
        ...
        if r.get(jobkey):
            logging.debug('Ignoring job {}'.format(job['data']))
        else:
            logging.info('Got job {}'.format(job['data']))
